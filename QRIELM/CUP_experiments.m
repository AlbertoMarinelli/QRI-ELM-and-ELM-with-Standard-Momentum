...
[w_new, B, R, H, u_matrix, final_num_neurons, history] = train (w, dataset, dataset_target, threshold, num_neurons, act_function, lambda, return_hist);
toc
...
% Our prediction
gap_pred = compute_gap ("relative_gap_pred", target, H_reg, B)
gap_sol = compute_gap ("relative_gap_sol", target, H_reg, B)
...
% Matlab QR error
gap_pred_mat = compute_gap ("relative_gap_pred", target, H_reg, B2)
gap_sol_mat = compute_gap ("relative_gap_sol", target, H_reg, B2)
